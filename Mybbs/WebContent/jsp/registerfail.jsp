...
<html>
	<body>
		...
		<div id="left">
			...
			-->
			<form:form modelAttribute="user"  action="${pageContext.request.contextPath}/user/addperson.do" method="post">
			<!--<form:errors path="*"/>-->
			<p>您是第一次来这里，请注册!</p>
				<p>账号(必填)：<form:errors path="account"  cssClass="errorClass"/>
										<form:input path="account"/></p>
				<p>密码(必填)：<form:errors path="password"  cssClass="errorClass"/>
										<form:input path="password" type="password"/></p>
				<p>确定密码  ：<input type="password" name="confirmpassword"/></p>		
								<p>姓名：<form:errors path="name"  cssClass="errorClass"/>
