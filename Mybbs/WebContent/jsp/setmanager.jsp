...
<html>
	<body>
		...
		<ul id ="welcome">
			...
			<c:set var="commonsnumber" value="${fn:length(commons)}"/>
			<c:if test="${commonsnumber!=0}">
			<c:forEach var="i"  begin="0"  end="${commonsnumber-1}">
			<tr>
				<td>${commons[i].account}</td>
				<td>普通</td>
				<td class="orange"><a href="javascript:if(confirm('确实要将这个用户设置为管理员吗?'))location='../bp/setmanager.do?bid=${bid}&pid=${commons[i].id}&oldname=${oldname }'">设置管理员</a></td>
			</tr>
			</c:forEach>
			</c:if>
			</table>
